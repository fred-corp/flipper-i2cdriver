...
static void cdc_send(const uint8_t* buf, uint16_t len) {
    while(len > 0) {
        ...
        buf += chunk;
        len -= chunk;
        if(len > 0) furi_delay_ms(2); // let USB hardware flush before next chunk
    }
}
...
static void crc_add(I2CDriverWorker* w, const uint8_t* data, size_t len) {
    uint16_t crc = w->crc;
    while(len--) crc = (crc_table[((crc>>8)^*data++)&0xff]^(crc<<8))&0xffff;
    w->crc = crc;
}
...
static void process_protocol(I2CDriverWorker* w) {
    ...
    while(rb_get(&cmd)) {
        ...
        if(furi_mutex_acquire(app->mutex, 5) == FuriStatusOk) {
            app->bytes_rx++;
            app->cmd_count++;
            app->usb_connected = true;  // we're receiving commands → host is connected
        ...
        if(cmd == 'd') {
            ...
            i2cdriver_log_push(app, "SCAN done");
            if(furi_mutex_acquire(app->mutex, 5) == FuriStatusOk) {
                app->bytes_tx += 112;
                furi_mutex_release(app->mutex);
            }
        ...
        if((cmd & 0xC0) == 0xC0) {
            ...
            i2cdriver_log_push(app, "W %dB %s", count, ok ? "ACK" : "NAK");
            if(furi_mutex_acquire(app->mutex, 5) == FuriStatusOk) {
                app->bytes_rx += count;
                app->bytes_tx++;
                furi_mutex_release(app->mutex);
        ...
        if((cmd & 0xC0) == 0x80) {
            ...
            crc_add(w, buf, count);
            cdc_send(buf, count);
            i2cdriver_log_push(app, "R %dB", count);
            if(furi_mutex_acquire(app->mutex, 5) == FuriStatusOk) {
                app->bytes_tx += count;
                furi_mutex_release(app->mutex);
            }
